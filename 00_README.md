...
> 운영 서비스: `https://hanpan.sodamlabs.kr`
>
> 내부 저장소·패키지·동의 식별자(`yut-review`, `com.yutreview`, `YUT_REVIEW`)는 호환성을 위해 유지한다.

## 프로젝트 개요
오프라인 매장에서 고객이 매장 전용 QR로 웹 윷놀이에 참여하고 차등 상품을 받는 이벤트 운영 서비스다. 네이버 플레이스 링크는 매장이 원할 때 함께 보여 주는 선택 동선이며, 리뷰 작성 여부는 참여나 혜택의 조건이 아니다.
...
- 상품 수량 제한 없음
- 사용정책: 당일 / 다음날 / 둘 다 허용
- 쿠폰 유효기간: 매장 설정(기본 90일, 1~365일). 쓸 수 있게 된 날부터 N일째 23:59:59까지이며,
  설정 변경은 그 이후 발급되는 쿠폰에만 적용된다.
- 대표 계정 1개가 여러 매장 관리 가능
- 매장 대표가 직접 회원가입해 매장을 등록하고, 같은 계정으로 매장을 더 추가할 수 있다
- 관리자 로그인은 이메일 + 비밀번호 (아이디 개념 없음)
- 휴대전화는 010 포함 숫자 11자리, 사업자등록번호는 숫자 10자리로 고정
- 선택 외부 링크: 네이버 플레이스(미설정 가능, 참여 조건으로 사용하지 않음)
- 향후 Solapi 카카오 알림톡 연동 예정
- 윷 연출: 모바일 게임형 60% + 프리미엄 미니멀형 40%

## 확정 요금제 정책

- BASIC 9,900원/월: 이벤트 운영 전체, 기본 집계, AI 매장 분석, 최근 90일 집계
- STANDARD 14,900원/월: BASIC 전체, 최근 365일 비교 분석, CSV 내려받기
- PRO 19,900원/월: STANDARD 전체, 안내물 브랜딩, AI 이벤트 문구·개선 제안·대화, 주간 자동 리포트, 기간 제한 없는 비식별 집계
- 모든 요금제에서 게임, QR, 상품, 쿠폰, 직원 PIN과 쿨타임은 동일하다.
- 무료체험은 가입일을 1일째로 14일간 PRO 전체 기능을 제공한다. 카드 선등록과 자동 결제는 없으며, 15일째 00:01(Asia/Seoul)에 BASIC으로 전환된다.
- 결제 PG는 아직 연동하지 않았고, 요금제 변경은 운영자 API로 처리한다.
