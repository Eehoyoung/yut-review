...
@Configuration class SecurityConfig {
    ...
    @Bean SecurityFilterChain chain(HttpSecurity http,JwtFilter jwt,OperatorAccessFilter operatorAccess) throws Exception {
        return http.csrf(c->c.disable()).sessionManagement(s->s.sessionCreationPolicy(SessionCreationPolicy.STATELESS))
            .authorizeHttpRequests(a->a.requestMatchers("/api/public/**","/api/admin/auth/login","/api/admin/auth/signup","/api/admin/auth/signup-requirements","/api/admin/auth/invite-code/*","/actuator/health").permitAll().anyRequest().authenticated())
